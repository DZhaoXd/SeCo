...
def val(val_loader, model, criterion, it, logger, writer):
    ...
    time1 = time.time()
    with torch.no_grad():
        for idx, (images, ins_mask, labels, name) in enumerate(val_loader):
            images = images.cuda()
            labels = labels.cuda()
...
def main():
    ...
    if cfg.distributed:
        ...
        train_sampler = None
    loader_dict['src_train'] = DataLoader(train_set, batch_size=cfg.batch_size, sampler=train_sampler,
                                          shuffle=(train_sampler is None), num_workers=cfg.num_workers, drop_last=True)
    ...
    if cfg.distributed:
        model.cuda()
        model = torch.nn.parallel.DistributedDataParallel(model, [dist.get_rank()])
    else:
    ...
    for it in range(start_iter, train_iters + 1):
        ...
        try:
            images, ins_mask, labels, true_labels, _, name = next(iter_source)
        ...
        ins_mask = ins_mask.cuda(non_blocking=True)
        bsz = images.shape[0]
        true_labels = true_labels['id'].cuda(non_blocking=True)
        ...
        true_acc1, true_acc5 = accuracy(output, true_labels, topk=(1, 5))  # acc use true labels
        top1.update(acc1[0], bsz)
        ...
        if it == start_iter or it % cfg.log_interval == 0:
            ...
            logger.info(f'Iter [{it}/{train_iters}] - '
                        ...
                        f'train[Acc@1:{top1.avg:.2f},Acc@5:{top5.avg:.2f}\t'
                        f'eval[Acc@1:{true_top1.avg:.2f},Acc@5:{true_top5.avg:.2f}')
            ...
            writer.add_scalar(f'Acc/src_eval', true_top1.avg, it)
        if it == start_iter or it % cfg.save_interval == 0:
            model_path = os.path.join(cfg.work_dir, 'iter_{}.pth'.format(it))
            state_dict = {
